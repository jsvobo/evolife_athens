#!/usr/bin/env python3
""" @brief 	 EVOLIFE: Scenario Labyrinth:
...
class Scenario(Default_Scenario):
	...
	def get_path(self, DNA):
		...
		for step in range(self['MaxSteps']):
			...
			if self.Poison[NewPosition[0]][NewPosition[1]] < self['MaxPoison']:
				self.Poison[NewPosition[0]][NewPosition[1]] += 1
		return ((step, walls, u_turns, poison), Positions)

	def eval_path(self, Path):
		(step, walls, u_turns, poison) = Path[0]
		return   self['PenaltyWall'] * (self['MaxSteps'] - walls) \
			   + self['PenaltyU_turn'] * (self['MaxSteps'] - u_turns)\
			   + self['PenaltyPoison'] * (self['MaxPoison'] * self['MaxSteps'] +1  - poison) \
			   + self['RewardExiting'] * (self['MaxSteps'] - step -1)
