#!/usr/bin/env python3
""" @brief  EVOLIFE: Scenario WernerDyer
"""
...
class Scenario(Default_Scenario):
	...
	def initialization(self):
		self.Ground = Grid(self['WDGridSize'])
	...
	def couples(self, members):
		...
		"""
		Amplification = self['Selectivity']   # to accelerate evolution
...
##            return MaleDirection    #test
	...
	def life_game(self, members):
		...
		"""
		self.PopSize10 = len(members) // 10
		Beloved = []
		# self.Ground.Consistency()

		for Female in members:
			Female.score(self['AgeMax'] - Female.age,FlagSet=True)  # all individuals deteriorate
		...
		self.Reproduction = 100 * sum(self.CurrentReproductionNumber,0.0)/max(1, 
			len(self.CurrentReproductionNumber))

	def Move(self, Male, Direction):
		" Male moves and possibily mates "

		if Male.location is None:	return
		if random.randint(0,99) < self['Noise']:
	...
	def display_(self):
		...
		"""
		return [('white','Reproduction'), ('green2','PopSize10')]
