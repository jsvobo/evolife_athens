...
def _writeGifToFile(fp, images, durations, loops):
    ...
    for im in images:
        ...
        if not previous:
            ...
            # write global header
            fp.write(byt(header))
            fp.write(byt(palette))
            fp.write(byt(appext))
            
            # write image
            fp.write(byt(graphext))
            fp.write(byt(imdes))
            for d in data:
                fp.write(byt(d))
...
def byt(s):	
	# return b''.join([bytes(x, encoding='latin-1') for x in s])
	if type(s) == str:	return bytes(s, encoding='latin-1')
	return s
