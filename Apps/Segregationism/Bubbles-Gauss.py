...
class Population:
	...
	def One_Decision(self):
		...
		# Use a mapping from influenced individual -> source location
		# Directly concerned agents are influenced by the film (source = film location)
		Concerned = {indiv: self.currentFilm.Location for indiv in DirectlyConcerned}

		for source in DirectlyConcerned:
			neigh = source.inspect(Type='individual', radius=Gbl['NeighbourhoodRadius'])
			...
			for nb in neigh:
				...
				if nb.Location is None:
					continue
				# Euclidean distance to the current source individual (not to the film)
				dx = nb.Location[0] - source.Location[0]
				dy = nb.Location[1] - source.Location[1]
				...
				# sample: if influenced, record the source location so the neighbour moves toward that individual
				if random.random() < p:
					Concerned[nb] = source.Location

		# Now move each concerned individual toward its recorded source location
		for indiv, target_loc in Concerned.items():
			indiv.closer(target_loc)
