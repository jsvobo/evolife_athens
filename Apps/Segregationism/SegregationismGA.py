...
class Scenario(ED.Default_Scenario):
	def __init__(self):
		...
		# Global variables		    #
		#############################
		AvailableColours = [4, 3, 6, 5, 7] + list(range(8, 21))
	...
	def evaluation(self, indiv):
		indiv.score(indiv.gene_value('SegGene2'), FlagSet=True)
	...
	def display_(self):
		...
		Note that 'Unweighted' is unsuitable to explore large space.
		"""
		return [('green1','SegGene1'),('red','SegGene2')]		 
	...
	def evaluation(self, indiv):
		indiv.score(indiv.gene_relative_value('SegGene1'))
...
class Settling_Observer(EO.EvolifeObserver):
	...
	def getData(self, Slot):
		if Slot == 'Positions':
			...
			self.CurrentChanges = []
			return tuple(CC)
		else:	return EO.EvolifeObserver.getData(self, Slot)
...
class Individual(EI.EvolifeIndividual):
	...
	def moves(self, Position=None):
		...
		if Position:
			...
			print("Unable to move to", Position, end='')
			return False
...
if __name__ == "__main__":
	...
	Land = Landscape.Landscape(Gbl.Parameter('LandSize'))	  # logical settlement grid
	...
	Observer.recordInfo('DefaultViews',	[('Field', 1000, 100, 400, 340),('Genomes')])	# Evolife should start with these windows open - these sizes are in pixels
