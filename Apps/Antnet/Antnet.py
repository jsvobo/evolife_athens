...
This program is still embryonic.
It's a straightforward application of the principles explained in
https://www.researchgate.net/profile/Gianni_Di_Caro/publication/2831144_AntNet_A_Mobile_Agents_Approach_to_Adaptive_Routing/links/0deec52909f324155c000000/AntNet-A-Mobile-Agents-Approach-to-Adaptive-Routing.pdf
"""
...
class Antnet_Observer(EO.Observer):
	...
	def getInfo(self, Slot, default=None):
		...
		if Slot == 'PlotOrders':
			...
			return [(10+M[0], 'Best current length for message #%d' % M[0]) for M in enumerate(list(self.MsgLength.keys()))]
...
class Network:
	...
	def __init__(self, Size=100, NbNodes=0, NetworkFileName=None):
		...
		if Gbl.Parameter('RandomNetwork') and NbNodes > 1:
			...
			self.NodeNames = {n.Name:n for n in self.Nodes}
...
class Ant:
	...
	def moves(self, Time, NbSteps):
		...
		if self.Age >= Gbl.Parameter('AgeMax'): self.NewAnt() 			# the ant is reborn
