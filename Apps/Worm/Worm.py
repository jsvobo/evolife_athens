...
class Worm_Observer(EO.Observer): 
	...
	def __init__(self, Scenario):
		...
		self.Trajectories = []
		self.MsgLength = dict()
		self.curve(Name='Time', Color='brown', Legend='distance to start location as time goes')	# declaration of a curve
...
class Worm:
	...
	def __init__(self, Observer, RuleString="0", speed=100, startPosition=[0,0]):
		...
		self.Grid = Grid(Observer, StartPosition)
...
class Grid:	
	...
	def __init__(self, Observer, StartPosition=[0,0]):
	...
	def eat(self, lastPosition, currentPosition):
		...
		DistanceToStart = abs(currentPosition[0] - self.Start[0]) +abs(currentPosition[1] - self.Start[1])
		Observer.curve('Time', DistanceToStart)
		return True
...
if __name__ == "__main__":
	...
	Observer.recordInfo('DefaultViews', [('Field', 800, 800)])
