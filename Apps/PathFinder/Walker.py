...
# Conversion of ground coordinates back to [0,100]
to100 = lambda Pos, Scale:	( Pos[0] * 100 // Scale[0], Pos[1] * 100 // Scale[1])
...
class Landscape(Landscape_.Landscape_3D):
	...
	def evaporation(self, Scale):
		for Pos in list(self.ActiveCells):
			if self.Cell(Pos).evaporate(): # no pheromone left
				...
				self.erase(Pos, Scale) # for ongoing display
				self.ActiveCells.remove(Pos)

	def erase(self, Pos, Scale):
		"""	says to Observer that there is no pheromone left at that location 
		"""
		Observer.record(('P%d_%d' % to100(Pos, Scale), to100(Pos, Scale) + (-3,))) # negative colour means erase from display
...
class Walker:
	...
	"""
	def __init__(self, IdNb, Scale=(100, 100), Start=(0,0), Target=(100,100)):
		self.ID = IdNb
		self.Scale = Scale
		self.Start = self.toGround(Start) # Starting position
		self.Target = self.toGround(Target)
		NoPoints = Gbl.Parameter('NoStartpoints')
		if NoPoints > 1:
			Y = int(IdNb[1:]) % NoPoints
			self.Start = (self.Start[0], 5 + int(Y * float(Land.Height - 10)/(NoPoints-1)))
			self.Target = (self.Target[0], Land.Height -5 - int(Y * float(Land.Height - 10)/(NoPoints-1)))
	...
	def toGround(self, Pos):
		"""	Converts coordinates from [0, 100] to ground coordinates
		"""
		return ( Pos[0] * self.Scale[0] //100, Pos[1] * self.Scale[1] //100 )
	
	def to100(self, Pos=None):
		"""	Converts ground coordinates back to [0,100]
		"""
		if Pos is None:	Pos = self.location
		return to100(Pos, self.Scale)
	...
	def moves(self):
		...
		if self.Action == 'BackHome':
			...
			Observer.record((self.ID, self.to100() + WalkerAspect)) # for ongoing display of Walkers
			...
			Observer.record(('P%d_%d' % self.to100(), self.to100() + PheromonAspect)) 
		else:
			NextPos = self.Sniff()
...
class Population:
	...
	"""
	def __init__(self, PopulationSize, Scale=(100, 100), Start=(0,0), Target=(100,100)):
		...
		for ID in range(PopulationSize):
			self.Pop.append(Walker('A%d' % ID, Scale=self.Scale, Start=self.Start, Target=self.Target))
...
if __name__ == "__main__":
	...
	Startpoint = tuple(map(int, Gbl.Parameter('Startpoint').split('x')))
	# Startpoint = ((int(Startpoint[0]) * Land.Width) // 100, (int(Startpoint[1]) * Land.Height) // 100)
	Endpoint = tuple(map(int, Gbl.Parameter('Endpoint').split('x')))
	# Endpoint =   ((int(Endpoint[0]) * Land.Width) // 100, (int(Endpoint[1]) * Land.Height) // 100)
	
	Pop = Population(Gbl.Parameter('PopulationSize'), Scale = (Land.Width, Land.Height), Start=Startpoint, Target=Endpoint)   # Walker colony
	# print(Land.Width, Land.Height)
	# Observer.record(('Rescale',(Land.Width, Land.Height, 0, 1)))	# to resize the field so that it matches the picture
